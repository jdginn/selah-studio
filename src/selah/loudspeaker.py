...
class Directivity:
    def __init__(
        self,
        horiz_disp: dict[float, float] = kh310_horiz_disp,
        vert_disp: dict[float, float] = kh310_vert_disp,
    ):
...
class LoudspeakerSpec:
    ...
    def gain(self, vert_angle: float, horiz_angle: float) -> float:
        ...
        Angles in degrees.
        """
        return self.directivity.gain(vert_angle, horiz_angle)
...
class SourceGenerator:
    ...
    def __init__(
        ...
        """
        SourceGenerator represents a generator of sources at a position
    ...
    def get_shot_from_angles(
        ...
        if np.allclose(self.position, listening_pos):
            normal = self.normal
        else:
        ...
        return Shot(
            self.position,
            sound.from_db(self.directivity.gain(pitch, yaw)),
            new_dir,
            self,
            shot_spec,
        )
    ...
    def get_shots(
        ...
        if np.allclose(self.position, listening_pos):
            ...
            dir = geometry.dir_from_points(self.position, listening_pos)
        ...
        h_step_size = dispersion_range / (h_steps - 1)
        v_steps = num_rays // h_steps
        v_step_size = dispersion_range / (v_steps - 1)
        for v in range(v_steps):
            pitch = -dispersion_range / 2 + v_step_size * v
            for h in range(h_steps):
                yaw = -dispersion_range / 2 + h_step_size * h
...
class Loudspeaker(SourceGenerator):
    # Reference normal against which all new normals are referenced
    ref_vec = np.array([-1, 0, 0])
    ...
    def __init__(
        ...
        """
        pos = position
        super(Loudspeaker, self).__init__(pos, normal, spec.directivity)
        self.spec = spec
        self._side = side
